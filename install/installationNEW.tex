...
\href{https://cism.github.io/}{CISM}\footnote{\texttt{https://cism.github.io/}} 
...
(e.g., \textit{Cheyenne} (CISL), \textit{Titan} (OLCF), and \textit{Edison} (NERSC) ). 
...
Other software dependencies include the \href{https://www.unidata.ucar.edu/software/netcdf/}{netCDF} library 
(used for data I/O) and a \href{https://www.python.org}{Python} distribution 
...
Linux (in blue boxes). For the former the instructions work for Mac OS High Sierra. For the latter, we have verified that the instructions work for Ubuntu 12.10 and 14.04 LTS.
...
As mentioned above, we will take advantage of \href{https://www.macports.org/}{MacPorts}, 
...
CISM has been tested extensively with \texttt{gfortran} version 6.
Newer (or older) versions may also work.  

\begin{mdframed}[style=mac] % V==============MAC================V
Searching for gcc with \texttt{port search gcc} will return:

\begin{verbatim}
apple-gcc40 @5495 (lang) 
...
with GCC 6.4.0 (others may work as well but have not been tested). 
To install GCC 6.4.0 type:

\texttt{sudo port install gcc6}
...
something like \texttt{gcc6 \@6.4.0\_0 (active)}. 
...
This confirms that GCC 6 is active (the \texttt{mp} indicates a Macports version). 
It is possible that gcc6 will be listed as active when you type \texttt{port installed}, 
but that mp-gcc6 will not be listed as active when you type \texttt{port select gcc}. 
If mp-gcc6 is not active as shown above, then you will need to select it using:
...
\begin{mdframed}[style=mac] % V==============MAC================V
To install NetCDF, use \texttt{sudo port install netcdf-fortran +gcc6}. 
Note that there are other versions of NetCDF available to install. It is important 
to choose the one with the ``Fortran" extension. The ``gcc6" syntax specifies a port ``variant". 
This tell Macports that, if there is a version of the selected software 
to install that is consistent with the GCC 6 compiler suite, then it should 
choose that one. Typing \texttt{port installed} should now include:

\begin{verbatim}
netcdf @4.4.1.1_1+dap+gcc6+netcdf4 (active)
netcdf-fortran @4.4.4_1+gcc6 (active)
\end{verbatim}

The ``dap+gcc6+netcdf4" comes along automatically. 
...
\begin{verbatim}
	sudo port install mpich-devel-gcc6 +fortran
\end{verbatim}
...
\begin{verbatim}
     mpich-devel-gcc6 @3.3a2_3+fortran (active)
\end{verbatim}
...
\begin{verbatim}
	sudo port select --set mpi mpich-devel-gcc6-fortran
\end{verbatim}
